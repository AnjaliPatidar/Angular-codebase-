...
export class SourceManagementComponent implements OnInit {
  ...
  getLanguage() {
    let getLanguageData = (language) => {
      ...
      let url = AppConstants.Ehub_Rest_API + 'fileStorage/downloadFileByLanguageAndFileName?fileName=' + param.fileName + '&languageName=' + param.languageName + '&token=' + param.token;
      this.translateService.setDefaultLang(AppConstants.Ehub_Rest_API + 'fileStorage/downloadFileByLanguageAndFileName?fileName=en_source_management.json&languageName=English&token=' + param.token);
