...
var Pietooltipdiv = d3.select("body").append("div").attr("class", "line_chart_tool").style("position", "absolute").style("z-index", 9992).style("background", "rgb(27, 39, 53)").style("padding", "5px 10px").style("visibility", "visible").style("border-radius", "10px").style("font-size", "10px").style("display", "none");
...
ReusablePie.prototype.reusableplotPie = function (pieOptions) {
    var asIndex = 0;
    ...
    if (pieOptions) {
        ...
        this.radiusDividedValue = pieOptions.radiusDividedValue ? pieOptions.radiusDividedValue : 2;
        this.innerTextWrap = pieOptions.innerTextWrap ? pieOptions.innerTextWrap : 2.5;
    ...
    var arc = d3.arc()
        ...
        .innerRadius(radius - radius / this.radiusDividedValue);
    var labelArc = d3.arc()
    ...
    g.append("path").style("fill", function (d, i) {
        ...
        .on("click", function (d) {
            ...
            else if (pieOptions.container == '#statusChart') {
              alert(1);
                if (d.data.key && d.data.key == 'successCount') {
                    document.getElementById('readPieChartSourceMonitoring_'+ d.data.columnID).setAttribute("data-id", 'Active');
                }
                else if (d.data.key && d.data.key == 'failedCount') {
                    document.getElementById('readPieChartSourceMonitoring_'+ d.data.columnID).setAttribute("data-id", 'Failed');
                }
                else {
                    document.getElementById('readPieChartSourceMonitoring_'+ d.data.columnID).setAttribute("data-id", 'InActive');
                }
                document.getElementById('readPieChartSourceMonitoring_'+ d.data.columnID).click()
            }
            else if (pieOptions.container == '#CaseByTypePie' || pieOptions.container == '#openCasePie') {
                document.getElementById('readPieChartSourceMonitoring_'+ d.data.columnID).setAttribute("data-id", d.data.statusObj.displayName);
                document.getElementById('readPieChartSourceMonitoring_'+ d.data.columnID).click()
            }
        ...
        .on("mouseover", function (d) {
            ...
            if (d.data.txt) {
                ...
                if (pieOptions.container == "#dataPart" || pieOptions.container == "#dayWeek") {
                    ...
                    Pietooltipdiv.html((d.data.key ? (d.data.key == 'successCount' ? pieOptions.localLanguageActive : (d.data.key == 'failedCount' ? pieOptions.localLanguageFailed : 'InActive Sources')) : '') + ": " + Number(d.value).formatAmt().split('.')[0]);
                }
                else {
        ...
        .on("mousedown", function (d) {
          if (pieOptions.onItemClick) {
            pieOptions.onItemClick(d);
          }
        })
        .transition()
        .delay(function (d, i) {
    ...
    if (pieOptions.istxt) {
        ...
        if (pieOptions.container == '#statusChart') {
            txtY = -11;
        }
        if(pieOptions.container == '#myAlertPie'){
            txtY = -7;
        }
        if (pieOptions.container == '#openCasePie' || pieOptions.container == '#CaseByTypePie') {
            txtY = 0;
        }

        if (typeof (pieOptions.istxt) != "boolean" && (pieOptions.istxt) != "true" && (pieOptions.istxt) != "false" && (pieOptions.showValue != 'noValue')) {
            g.append("text").attr("y", txtY).text((pieOptions.container && (pieOptions.container == '#statusChart' || pieOptions.container == '#myAlertPie')) ? pieOptions.istxt + ' ' +  (pieOptions.istxtLabel || 'Total') : pieOptions.istxt).style('text-anchor', "middle").style('font-size', this.txtSize).style('width', '90px').style("fill", this.txtColor).style("stroke", "none").call(wrap, 50);
...
function wrap(text, width) {/*jshint ignore:line*/
    text.each(function () {
        ...
        while (word = words.pop()) {/*jshint ignore:line*/
            ...
            if (tspan.node().getComputedTextLength() > width) {
                ...
                line = [word];
                if (lineNumber == 0) {
                    tspan.style("font-size", "34px").style("padding-bottom", "13px");
                }

                tspan = text.append("tspan").attr("x", x).attr("y", y).attr("dy", ++lineNumber+0.5 * lineHeight + dy + "em").text(word);
                if (lineNumber == 1) {
                    tspan.style("font-size", "12px").attr("y", 1).style("fill", "#657F8B").style("padding-top", "13px");
                }
            }
        }
    });
